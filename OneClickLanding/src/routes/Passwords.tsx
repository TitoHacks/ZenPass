...
import {Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Divider, Chip, ChipProps, Image, Progress, Button} from "@nextui-org/react";
import PasswordDialog from '@/components/ui/password-dialog'
import {Modal} from "@nextui-org/react";
import { getEntries } from '@/utils/utils'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { IconDefinition, faCopy, faEye, faEyeSlash, faFile, faMagnifyingGlass, faPlus } from '@fortawesome/free-solid-svg-icons'
    ...
    async function fetchData() {
      ...
      entries.forEach((entry)=>{
        ...
        if(entry.title.toLowerCase().includes(searchValue.toLowerCase())){
          passwordComponents.push(<PasswordComponent passwordItem={entry} setDetail={setDetail} setPasswordItem={setPasswordItem} favicon={entry.favicon} status={entry.status} title={entry.title} url={entry.url}></PasswordComponent>)
        }
        
      })
      if(passwordComponents.length == 0){
        passwordComponents = [<p className='text-gray-500 justify-self-center'>Añade tu primera contraseñas pulsando el boton "+" en la parte inferior derecha.</p>]
      }
      ...
      <div className='flex flex-row justify-between h-screen bg-backgroundColor w-full dark'>
        <div className='ml-40 pt-12 h-full w-full flex flex-col items-center '>
          <div className='self-start flex flex-row n w-2/4 justify-between pb-4'>
            <h1 className='text-gray-200 font-bold text-2xl ml-10 self-start'>Contraseñas</h1>
            <Input id='searchBar' label="Search" className='w-2/4 text-white' variant='flat' color='default' onKeyUp={function(){setSearched(!searched)}} isClearable radius="lg" placeholder="Type to search..." startContent={<FontAwesomeIcon icon={faMagnifyingGlass} />}/>
          </div>
        ...
        <div className='absolute bottom-4 right-4'>
          <Dropdown backdrop="blur">
            <DropdownTrigger>
              <Button variant="flat" isIconOnly={true} >+</Button>
            </DropdownTrigger>
            <DropdownMenu color='danger' variant="flat" aria-label="Static Actions">
              ...
              <DropdownItem key="copy" onClick={function(){setOpenCsv(true)}} startContent={<FontAwesomeIcon icon={faFile} />}> Import password from .csv</DropdownItem>
            </DropdownMenu>
          </Dropdown>
        </div>
